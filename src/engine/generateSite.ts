...
// TowerSite/ShrineSite/RiftSite/KeepSite are part of the union so the type system and UI can
// already handle them, but generateSiteForHex doesn't dispatch to them yet — the full Points of
// Interest table rewrite (routing all 5 new site kinds) is deliberately batched until Tower,
// Shrine, Rift, Keep, and Camp all exist, rather than rewiring this dispatch once per kind.
// See docs/plan-sites-settlements-mongo.md's "Location Generator" section.
export type GeneratedSite = DungeonSite | Settlement | TowerSite | ShrineSite | RiftSite | KeepSite
