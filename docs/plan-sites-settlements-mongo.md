...
7. ✅ **Real settlement maps** — Voronoi districts clipped to an organic jittered town boundary, curved road network, replacing the uniform grid-blob settlement districts. New geometry-library dependencies (`d3-delaunay` + `polygon-clipping`) — first such dependencies in this project. See "Real settlement maps" section below for what shipped (including a scope simplification that avoided needing `turf.js`) — Done.
